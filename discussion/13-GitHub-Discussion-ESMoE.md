...
| **MoE+MoT Shared v08_moe_mot_shared** | 官方 Issue #54 跨尺度共享实验（MoE+MoT, P3/P4 共享 expert pool）| — | 16.80% | 29.52% | 38.47% | 31.36% | — |
...
| **参数量** | **−10.4%**（2.814M vs 3.14M）| **−24.2%**（2.814M vs 3.71M）| −12.9% | **（未实测）** best.pt 参数量未在本项目读出；理论上 MoT 重型 Transformer 专家共享应带来更大削减 |
...
5. ✅ **ES-MoE K=2 参数量最少**（2.814M，比 MoE −10.4%、比 MoT −24.2%、比 MoA −12.9%；官方 MoE+MoT Shared best.pt 参数量未实测）—— 多尺度 DWconv 专家是当前最紧凑的 MoE 实现
7. ✅ **ES-MoE K=2 训练时长中等**（13.0h，比 MoE −0.3h、比 MoT −33.3%、比 MoA +2.1h）—— 性价比最高的"速度-精度"折中
9. ✅ **SharedExpertESMoE vs 官方 MoE+MoT Shared（v08_moe_mot_shared）的诚实对比**：本项目 SharedExpertESMoE 用 DWconv 轻量专家做 P4/P5 共享（**实测参数−3.1%，2.814M→2.727M**，Expert 对象 16→12）；官方 v08_moe_mot_shared 用 Transformer 重型专家做 P3/P4 共享，best.pt 真实参数量本项目未单独读出，**理论上 Transformer 专家共享空间更大**。**轻量专家共享空间小是架构天然属性，非实现缺陷**——这是有价值的负面结论，已在 §6 详述
