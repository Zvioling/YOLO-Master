...
1. ✅ **ES-MoE K=2 是 7 个变体中 mAP 最高**（17.27%）—— 比官方 MoE v084 (+0.43pp)、MoT (+0.34pp)、MoA (+0.47pp) **均有正增益**
3. ✅ **ES-MoE K=2 是 7 个变体中 Precision 最高**（43.48%）—— 比 v084 +4.01pp、MoT +4.11pp、MoA +2.73pp，**说明多尺度专家（3×3/5×5/7×7/9×9）协同让模型更聚焦正样本**
5. ✅ **ES-MoE K=2 参数量最少**（2.814M，比 MoE −10.4%、比 MoT −24.2%、比 MoA −12.9%）—— 多尺度 DWconv 专家是当前最紧凑的 MoE 实现
7. ✅ **ES-MoE K=2 训练时长中等**（13.0h，比 MoE −0.3h、比 MoT −33.3%、比 MoA +2.1h）—— 性价比最高的"速度-精度"折中
9. ⚠️ **同架构内 K 选择很关键**：K=1（16.13%）、K=3（15.20%）、Shared（15.16%）均不如 K=2——证明 K=2 是 ES-MoE 最优拓扑
